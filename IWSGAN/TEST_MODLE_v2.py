# -*- coding: utf-8 -*-
"""
Created on Sat Apr 17 16:48:54 2021
...
class Net_models():
    ...
    def build_model(self):
        ...
        discriminator = self.build_discriminator(self.feature)
        discriminator.compile(optimizer = 'adam',loss = self.wasserstein_loss)
        discriminator.trainable = False
        real_img = Input(shape =self.img_shape)
        vaild = discriminator(real_img)
        gan_image = generator(real_img)
        ...
        return gan_trainer,discriminator,generator
    ...
    def build_featrue(self):
        ...
        E3 = self.encoder_layer(P_2,128,BN=True,activation='leakyReLU')
        P_3 = AveragePooling2D((2, 2))(E3)
        E4 = self.encoder_layer(P_3,100,BN=True,Drop=True,activation='leakyReLU')
        f = GlobalAveragePooling2D(name='glb_avg_1')(E4)
        f = LeakyReLU(alpha=0.1)(f)
        # f = Activation('relu')(f)
        # d = Dense(100,name = 'd_out',activation = 'relu')(f)
        # P_4 = AveragePooling2D((2, 2))(E4)
    ...
    def train(self,batch_size,epochs,sample_interval):
        gan_trainer,discriminator,generator = self.build_model()
        valid = np.ones((batch_size,1))+0.1 *(0.9)+(0.1/2)#更改過本來為+0.1 *(0.9)+(0.1/2)
        fake = -np.ones((batch_size,1))
        myGene = trainGenerator()
        for epoch in range(epochs):
            for _ in range(self.n_critic):
                x = myGene.__next__()
                fake_images = generator.predict(x)
    ...
    def evaluation(self,feature,generator,images,labels):
        ...
        print('lat_mean.shape:',lat_mean.shape)
        # lat = np.square(real_feature-gan_feature)
        score = 0.9*rec_mean+0.1*lat_mean
        score = (score - np.min(score))/(np.max(score)-np.min(score))
        X_tsne = manifold.TSNE(n_components = 3,init='random',
                               random_state=5,verbose=1).fit_transform(gan_feature)
    ...
    def plot_all_image(self,score,labels,real_images,gan_images):
        ...
        plt.hist(one_number,bins = 30,label='abnormal',alpha = 0.5,color = 'r',stacked = True,density=True)
        plt.hist(zero_number,bins = 30,label='normal',alpha = 0.5,color = 'b',stacked = True,density=True)
        plt.legend(loc='upper left')
        
        print('one_number.shape:',one_number.shape)
        print('zero_number.shape:',zero_number.shape)
        fpr,tpr,_ = roc_curve(labels, score,pos_label = 1)
        roc_auc = auc(fpr,tpr)
        f = open('read_roc.txt','a')
        # read_f= f.read()
        # print(read_f)
        x = str(roc_auc)+','
        # '\n'+'fpr'+str(fpr)+'\n'+'tpr'+str(tpr)
        ...
        plt.figure(5)
        # print(gan_images[1])
        # print('Max:',np.array(gan_images[1]).max)
        # print('Min:',np.array(gan_images[1]).min)
        # len(gan_images)
        
        # for i in range(len(gan_images)):
        #     fig = plt.imshow(gan_images[i].reshape(128,128),cmap='gray')
        #     fig.axes.get_xaxis().set_visible(False)
        #     fig.axes.get_yaxis().set_visible(False)
        #     plt.savefig('GAN_Images/%d.png'%i,bbox_inches='tight',pad_inches=0,dpi = 42)
        # plt.show()
        roc_auc = 0
    def sample_images(self,epoch,x,generator):
        gan_imgs = generator.predict(x)
        ...
        plt.close()
    def save_model(self,generator,discriminator,feature,gan,path):
        generator_path = path + '_generator.h5'
        discriminator_path = path + '_discriminator.h5'
        feature_path = path + '_feature.h5'
        gan_path = path + '_gan.h5'
...
if __name__ == '__main__':
    UNET_GAN = Net_models()
    feature = UNET_GAN.build_featrue()
    feature.summary()
