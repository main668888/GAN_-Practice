# -*- coding: utf-8 -*-
"""
Created on Thu Apr 15 22:32:49 2021
...
class Net_models():
    ...
    def decoder_layer(self,inputs,paired_inputs,filters,connect,kernel_size=3):
        x = inputs
        x = Conv2D(filters, 2, padding = 'same', kernel_initializer = 'he_normal',activation='relu')(x)
        # x = LeakyReLU(alpha=0.2)(x)
        if connect==True:
             x = concatenate([paired_inputs,x])
        x = Conv2D(filters,kernel_size,padding='same'
                        ,kernel_initializer = 'he_normal',
                        bias_initializer = 'zero',activation='relu')(x)
        # x = LeakyReLU(alpha=0.2)(x)
        # x = Activation('relu')(x)
        return x
    def build_generator(self):
        img = Input(shape = self.img_shape,name = 'input_layer')
        E1 = self.encoder_layer(img,32,BN=False,activation='relu')
        P_1 =AveragePooling2D((2, 2))(E1)
        E2 = self.encoder_layer(P_1,64,BN=True,activation='relu')
        P_2 = AveragePooling2D((2, 2))(E2)
        E3 = self.encoder_layer(P_2,128,BN=True,activation='relu')
        P_3 = AveragePooling2D((2, 2))(E3)
        E4 = self.encoder_layer(P_3,256,BN=True,Drop=True,activation='relu')
        ...
        C_1 = Conv2D(2,3,padding='same',activation='relu')(D_4)#20210331改過原來 是ReLU
    ...
    def CalculationLR(self,epoch,lr_g,lr_d):
        if epoch == 500 or epoch == 750 or epoch == 875 or epoch == 950:
            lr_g = 0.5*lr_g
            lr_d = 0.5*lr_d
        return lr_g,lr_d
    ...
    def sample_images(self,epoch,x):
        ...
        for i in range(16):
            ...
            ax.get_xaxis().set_visible(False)
            ax.get_yaxis().set_visible(False)
        plt.savefig("images_test_relu/A_real_images_%d.png" % epoch)
        plt.close()
