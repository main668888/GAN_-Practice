# -*- coding: utf-8 -*-
"""
Created on Tue Mar 31 14:22:28 2020
...
if args.mode == 'train':
    Model_d, Model_g = AnoGANmodel2.train(64, X_train)

### 2. test generator
generated_img = AnoGANmodel2.generate(25)
img = AnoGANmodel2.combine_images(generated_img)
img = (img*127.5)+127.5
...
def anomaly_detection(test_img, g=None, d=None):
    model = AnoGANmodel2.anomaly_detector(g=g, d=d)
    ano_score, similar_img = AnoGANmodel2.compute_anomaly_score(model, test_img.reshape(1, 28, 28, 1),
                                                               iterations=500, d=d)
    ...
    diff = cv2.addWeighted(original_x_color, 0.3, residual_color, 0.7, 0.)

    return ano_score, original_x, similar_x, diff
...
model = AnoGANmodel2.feature_extractor()
